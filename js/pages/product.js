...
function renderWeightOptions() {
  if (!weightOptionsEl) return;

  weightOptionsEl.innerHTML = "";

  const availableWeights = allVariants.filter(
    (variant) => (variant.flavor || "Без вкуса") === selectedFlavor
  );
  ...
  availableWeights.forEach((variant) => {
    ...
    btn.addEventListener("click", () => {
      ...
      selectedVariant = allVariants.find(
        (item) =>
          (item.flavor || "Без вкуса") === selectedFlavor &&
...
function renderFlavorOptions() {
  if (!flavorOptionsEl) return;

  flavorOptionsEl.innerHTML = "";

  const uniqueFlavors = [
    ...new Set(allVariants.map((variant) => variant.flavor || "Без вкуса"))
  ];
  ...
  uniqueFlavors.forEach((flavor) => {
    ...
    btn.addEventListener("click", () => {
      ...
      const weightsForFlavor = allVariants.filter(
        (variant) => (variant.flavor || "Без вкуса") === flavor
      );
...
async function loadReviews() {
  ...
  if (openReviewFormBtn) {
    openReviewFormBtn.style.display = myReview
      ? "none"
      : currentUser
      ? "inline-flex"
      : "none";
